...
class PostListViewTests(APITestCase):
    def setUp(self):
        User.objects.create_user(username='max', password='password')

    def test_can_list_posts(self):
        max = User.objects.get(username='max')
        Post.objects.create(owner=max, title='a title')
...
class PostDetailViewTests(APITestCase):
    def setUp(self):
        max = User.objects.create_user(username='max', password='password')
        john = User.objects.create_user(username='john', password='password')
        Post.objects.create(
            owner=max, title='title1', content='content1'
        )
        Post.objects.create(
